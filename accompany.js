/*
 * $Id: accompany.js,v 0.2 2013/03/14 11:48:51 dankogai Exp dankogai $
 *
...
(function(global) {
    ...
    var getConstructorOf = function(o) {
        ...
        if (proto) {
            return isFunction(proto) ? proto : proto === Object.prototype ?
                Object : getConstructorOf(proto);
        }
        return null;
    };
    var clone = function(o, deep) {
        ...
        // Arrays
        if (Array.isArray(o)) {
            return !deep ? slice.call(o) : o.map(function(v) {
                return clone(v, deep);
            });
        }
    ...
    defaults(Object, newSpecs({
        ...
        has: function(o, k) { return hasOwnProperty.call(o, k) },
        is: is,
        isnt: function(x, y) { return !is(x, y) },
        typeOf: typeOf,
        isObject: isObject,
    ...
    defaults(Boolean, newSpecs({
        isBoolean: function(s) { return typeOf(s) === 'boolean' }
    }));
    ...
    defaults(Number, newSpecs({
        ...
        isFinite: function(n) { return n === 1 * n && gIsFinite(n) },
        isNumber: function(n) { return typeOf(n) === 'number' },
        isInteger: function(n) { return Number.isFinite(n) && n % 1 === 0 },
    ...
    defaults(Math, newSpecs({
        acosh: function(n) { return Math.log(n + Math.sqrt(n * n - 1)) },
        asinh: function(n) { return Math.log(n + Math.sqrt(n * n + 1)) },
        atanh: function(n) { return 0.5 * Math.log((1 + n) / (1 - n)) },
        cbrt: function(n) { return Math.pow(n, 1 / 3) },
        cosh: function(n) { return (Math.exp(n) + Math.exp(-n)) / 2 },
        expm1: function(n) { return Math.exp(n) - 1 },
        hypot: function(x, y) { return Math.sqrt(x * x + y * y) || +0 },
        log2: function(n) { return Math.log(n) / Math.LN2 },
        log10: function(n) { return Math.log(n) / Math.LN10 },
        log1p: function(n) { return Math.log(1 + n) },
        sign: function(n) {
            n *= 1;
