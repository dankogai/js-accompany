...
describe('Map truthy primitives', function () {
    ...
    it('m.size', is(m.size, truthy.length));
    it('m.keys()', is_deeply(m.keys().sort(), truthy));
    it('m.values()', is_deeply(m.values().sort(), truthy));
    it('m.items()', is_deeply(m.items().sort(),
    truthy.map(function (v) {
        return [v, v]
    }).sort()));
    for (i = 0, l = truthy.length; i < l; ++i) {
        v = truthy[i];
